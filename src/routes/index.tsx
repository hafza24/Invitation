...
import rukhsatiImg from "@/assets/rukhsati.jpg";

export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: `${siteConfig.wedding.bride} & ${siteConfig.wedding.groom} — Wedding Invitation` },
      {
        name: "description",
        content: `Join us in celebrating the wedding of ${siteConfig.wedding.bride} & ${siteConfig.wedding.groom} on ${siteConfig.wedding.dateLabel}.`,
      },
      { property: "og:title", content: `${siteConfig.wedding.bride} & ${siteConfig.wedding.groom}` },
      { property: "og:description", content: siteConfig.wedding.tagline },
    ],
...
function Hero() {
  ...
  const title = "A Celebration of Love";
  return (
    <section ref={ref} className="relative h-screen overflow-hidden bg-gradient-hero">
      ...
      <motion.div style={{ opacity }} className="relative z-10 flex h-full flex-col items-center justify-center px-6 text-center">
        ...
        >
          ...
          <span className="font-script text-5xl sm:text-7xl text-gradient-gold">
            {siteConfig.wedding.bride} & {siteConfig.wedding.groom}
          </span>
      ...
      <motion.div
        ...
        className="absolute bottom-10 left-1/2 -translate-x-1/2 text-gold/60 text-xs uppercase tracking-[0.4em]"
      >
        Scroll
      </motion.div>
    </section>
  ...
  return (
    <section className="relative py-32 px-6 overflow-hidden">
      <motion.div {...fadeUp} className="max-w-5xl mx-auto text-center">
        ...
        <p className="text-muted-foreground italic max-w-2xl mx-auto">{siteConfig.family.blessing}</p>
      </motion.div>
      <div className="mt-20 max-w-5xl mx-auto grid md:grid-cols-2 gap-8">
        {[
          {
            title: "The Bride",
            name: siteConfig.wedding.bride,
            label: "Daughter of",
            father: siteConfig.family.brideFather,
            mother: siteConfig.family.brideMother,
          },
          {
            title: "The Groom",
            name: siteConfig.wedding.groom,
            label: "Son of",
            father: siteConfig.family.groomFather,
            mother: siteConfig.family.groomMother,
          },
        ].map((f, i) => (
          <motion.div
            key={f.title}
...
function EventSection({
  ...
  event: (typeof siteConfig.events)[number];
  image: string;
  reverse?: boolean;
  themeClass: string;
}) {
  return (
    <section className={`relative py-32 px-6 overflow-hidden ${themeClass}`}>
      <Particles count={20} color="var(--gold)" />
      <div className={`relative max-w-6xl mx-auto grid md:grid-cols-2 gap-12 items-center ${reverse ? "md:[direction:rtl]" : ""}`}>
        ...
        <motion.div {...fadeUp} className="[direction:ltr]">
          ...
          <a
            ...
            className="mt-10 inline-flex items-center gap-2 px-8 py-3 bg-gradient-gold text-primary-foreground rounded-full font-medium hover:shadow-gold transition-all hover:scale-105"
          >
            View on Map →
          </a>
        </motion.div>
  ...
  return (
    <section ref={ref} className="relative h-[140vh] overflow-hidden">
      <motion.div style={{ y }} className="absolute inset-0">
        <img src={rukhsatiImg} alt="" className="h-full w-full object-cover" />
  ...
  return (
    <section className="relative py-32 px-6">
      <motion.div {...fadeUp} className="max-w-3xl mx-auto text-center mb-16">
        ...
        <p className="mt-4 text-muted-foreground italic">A note for the couple to treasure.</p>
      </motion.div>
      <form onSubmit={submit} className="max-w-2xl mx-auto glass-gold rounded-2xl p-8 space-y-4">
        <input
          ...
          maxLength={80}
          className="w-full bg-transparent border-b border-border py-3 px-2 text-ivory placeholder:text-muted-foreground focus:outline-none focus:border-gold transition-colors"
        />
        <textarea
          value={message}
          onChange={(e) => setMessage(e.target.value)}
          placeholder="Your blessing for Bushra & Musa…"
          rows={4}
          maxLength={300}
          className="w-full bg-transparent border-b border-border py-3 px-2 text-ivory placeholder:text-muted-foreground focus:outline-none focus:border-gold transition-colors resize-none"
        />
        <button
          type="submit"
          className="w-full py-3 bg-gradient-gold text-primary-foreground rounded-full font-medium hover:shadow-gold transition-all"
        >
          Send Wish
        </button>
      </form>
  ...
  return (
    <section id="rsvp" className="relative py-32 px-6">
      <motion.div {...fadeUp} className="max-w-3xl mx-auto text-center mb-16">
        ...
        <p className="mt-4 text-muted-foreground italic">Your seat at our table awaits.</p>
      </motion.div>
      <form onSubmit={submit} className="max-w-2xl mx-auto glass-gold rounded-2xl p-8 sm:p-10 space-y-5">
        ...
        <select value={form.event} onChange={(e) => setForm({ ...form, event: e.target.value })} className="w-full bg-transparent border-b border-border py-3 px-2 focus:outline-none focus:border-gold">
          ...
          {siteConfig.events.map((ev) => (<option key={ev.id} className="bg-card">{ev.name}</option>))}
        </select>
        ...
        <button type="submit" className="w-full py-4 bg-gradient-gold text-primary-foreground rounded-full font-medium hover:shadow-gold transition-all hover:scale-[1.02]">
          Confirm Attendance
        </button>
      </form>
    </section>
        ...
        {siteConfig.contacts.map((c) => (
          <motion.div key={c.label} {...fadeUp} className="glass-gold rounded-2xl p-8">
            ...
            <div className="flex flex-wrap gap-3">
              <a href={`tel:${c.phone}`} className="px-5 py-2 rounded-full glass text-sm hover:bg-gold/10 transition-colors">Call</a>
              <a href={`https://wa.me/${c.whatsapp.replace(/[^\d]/g, "")}`} target="_blank" rel="noopener noreferrer" className="px-5 py-2 rounded-full glass text-sm hover:bg-gold/10 transition-colors">WhatsApp</a>
            </div>
          </motion.div>
        ))}
      </div>
  ...
  return (
    <section className="relative py-32 px-6 bg-gradient-to-b from-transparent via-card/30 to-transparent">
      ...
      <motion.div {...fadeUp} className="relative text-center mb-16">
        ...
        <p className="mt-4 text-muted-foreground italic">{siteConfig.wedding.dateLabel}</p>
      </motion.div>
      <Countdown date={siteConfig.wedding.date} />
    </section>
  );
}
...
function WeddingPage() {
  return (
    <main className="bg-background text-foreground">
      <Hero />
      <FamilySection />
      <EventSection event={siteConfig.events[0]} image={mehndiImg} themeClass="bg-gradient-to-b from-transparent via-marigold/5 to-transparent" />
      <EventSection event={siteConfig.events[1]} image={baratImg} reverse themeClass="bg-gradient-to-b from-transparent via-barat/10 to-transparent" />
      <EventSection event={siteConfig.events[2]} image={walimaImg} themeClass="bg-gradient-to-b from-transparent via-gold/5 to-transparent" />
      <Rukhsati />
      <CountdownSection />
      <Wishes />
      <RSVP />
